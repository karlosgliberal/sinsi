...
const itemsEstadistica = [
  ...
  {reaccion: 'estadisticaReaccionGenero', pregunta: 'estadisticaPreguntaGenero'},
  {reaccion: 'estadisticaReaccionColor', pregunta: 'estadisticaPreguntaColor'}
];
let timer;
...
export default function Chat() {
  ...
  const [botonSaltoTemporalActivated, setBotonSaltoTemporalActivate] = useState(false);
  ...
  const getIntention = async intention => {
    ...
    let continuar = controlConversacion(resIntentName, fulfillmentText, fallback);
  ...
  const getTipoIntencion = (resIntentName) => {
    if (resIntentName.indexOf('pregunta') !== -1 || resIntentName.indexOf('Pregunta') !== -1 || resIntentName.indexOf('corte') !== -1) {
      return 'pregunta';
    }
    return 'respuesta';
  };
  ...
  const avisoInactividad = (resIntentName) => {
    ...
    if(tipoIntencion == 'pregunta'){
      ...
      if(resIntentName.indexOf('provocaUsuario') !== -1 || resIntentName.indexOf('corteTiempo') !== -1){
  ...
  const controlPreguntasEstadistica = (resIntentName) => {
    let resultado = itemsEstadistica.findIndex( estadistica => estadistica.reaccion === resIntentName);
  ...
  const futuroReaccionSaltoTemporal = (fulfillmentText) => {
    ...
    let res = fulfillmentText.replace("%futuroReaccionSaltoTemporal%", opcionSel);
  ...
  const futuroPreguntaDesencadenante = (fulfillmentText) => {
    let res = fulfillmentText.replace("%futuroPreguntaDesencadenante%", localStorage.getItem('futuroReaccionSaltoTemporal'));
    return res;
  }
  ...
  const futuroReaccionDesencadenante = (fulfillmentText) => {
    ...
    let res = fulfillmentText.replace("%futuroReaccionDesencadenante%", opcionSel);

    return res;
  }

  const futuroPreguntaTipoFuturo = (fulfillmentText) => {
    let res = fulfillmentText.replace("%futuroPreguntaTipoFuturo%", localStorage.getItem('futuroReaccionSaltoTemporal'));
    return res;
  }
  ...
  const futuroPreguntaTema = (fulfillmentText) => {
    let res = fulfillmentText.replace("%futuroPreguntaTema%", localStorage.getItem('futuroReaccionSaltoTemporal'));
    return res;
  }
  ...
  const futuroPreguntaLugar = (fulfillmentText) => {
    let resumen = "Hemos dado un salto temporal de "+localStorage.getItem('futuroReaccionSaltoTemporal')+". Tras "+localStorage.getItem('futuroReaccionDesencadenante')+", "+localStorage.getItem('futuroReaccionPoblacion')+" están viviendo un futuro "+localStorage.getItem('futuroReaccionTipoFuturo')+", donde, en el área "+localStorage.getItem('futuroReaccionSector')+", el tema más comentado será "+localStorage.getItem('futuroReaccionTema')+".";
